...
local UI_Erros = {
	...
	[_G.SPELL_FAILED_PREVENTED_BY_MECHANIC] = function(GUID) blackListInfront(GUID) end,
	[_G.SPELL_FAILED_NOPATH] = function(GUID) blackListInfront(GUID) end,
	[_G.SPELL_FAILED_LINE_OF_SIGHT] = function(GUID) blackListInfront(GUID) end,
	[_G.SPELL_FAILED_VISION_OBSCURED] = function(GUID) blackListInfront(GUID) end,
}
...
function NeP.Helpers.Spell(_, spell, target, GUID)
	...
	if _Failed[spell] then
		 return not _Failed[spell]
	end
	return true
end
